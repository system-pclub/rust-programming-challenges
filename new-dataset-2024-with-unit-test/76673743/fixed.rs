...
mod tests {
    ...
    fn test_entity() {
        ...
        assert_eq!(sub_.states.iter().map(|state| state.get()).collect::<Vec<_>>(), vec![false, false, false]);
        ...
        assert_eq!(sub_.states.iter().map(|state| state.get()).collect::<Vec<_>>(), vec![true, true, true]);
    }
}

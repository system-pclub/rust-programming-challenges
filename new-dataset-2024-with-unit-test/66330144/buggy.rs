...
impl<T: Ord> Tree<T> {
    ...
    fn remove(&mut self, value: &T) {
        ...
        while let Some(ref mut node) = current {
            match node.key.cmp(value) {
                Ordering::Less => current = &mut node.right.0,
                Ordering::Greater => current = &mut node.left.0,
                Ordering::Equal => {
                    match (node.left.0.take(), node.right.0.take()) {
                        ...
                        (Some(left), Some(right)) => {
                            let mut right_tree = Tree(Some(right));
                            let min_value = right_tree.extract_min().unwrap();
